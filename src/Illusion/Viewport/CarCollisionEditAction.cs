...
/// <summary>
/// One collision-level intent on the undo stack — however many structures it touched.
///
/// <para>
/// Adding a solid collision writes a prefab volume, an ItemDesc record, a manifest entry, a frame stub and a
/// handful of hit boxes. That is ONE thing the modder did, so it is one Ctrl+Z, and what comes back is the
/// SNAPSHOT the aggregate took rather than a reversed derivation: the hit-box rule reproduces 65.3 % of the
/// boxes a car ships with, so reversing it would leave the other third rebuilt and the car quietly different
/// from the one the modder started with.
/// </para>
/// </summary>
internal sealed class CarCollisionEditAction : IEditAction
{
    private readonly CarCollisionEdit _edit;
    private readonly Func<Car?> _car;
    private readonly Action _after;
    private readonly Action<string> _refused;
    private readonly string? _prefab;

    /// <param name="car">The car as it stands NOW. The stubs and hit boxes in the snapshot belong to the
    ...
    internal CarCollisionEditAction(
        CarCollisionEdit edit, Func<Car?> car, Action after, Action<string> refused)
    {
        _edit = edit;
        _car = car;
        _after = after;
    ...
    private void Apply(CarCollisionState want, CarCollisionState back)
    {
